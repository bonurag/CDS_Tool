...
# We will regex replace from sync function computeOptimal() down to its closing bracket brace before // ── CLASSIFICA.
start = text.find('async function computeOptimal(){')
end = text.find('// ── CLASSIFICA')

if start != -1 and end != -1:
    old_js = text[start:end]
    new_js = '''async function computeOptimal(){
  ...
  try {
      ...
      const payload = {
          ...
          data: active.map(r => ({
              // Passa tutto ma sovrascrivi pts con quello UI
              ...r,
              pts: pts(r)
          }))
      };
      ...
      if (!res.optimal || !res.optimal.sel || res.optimal.sel.length !== C.nSel){
          ...
          setNoteEst('');
          staffAnalysis = []; // Opzionale: disabilitato analysis o calcolato
          res.optimal.sel.forEach(r => selectedIds.add(String(r.id)));
      }
...
'''
    ...
    with open('fidal_cds_tool.py', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Replaced JS implementation!")
